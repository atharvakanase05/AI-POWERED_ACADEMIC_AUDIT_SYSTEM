...
//               <Card title="📄 Publications Analysis" />
//             </Link>
//           </div>
//         </div>
//       </div>
...
function Dashboard() {
  // ✅ KPI States (100 students base)
  const [isAnalyzing, setIsAnalyzing] = useState(false);
  const [totalStudents, setTotalStudents] = useState(100);
  const [passPercentage, setPassPercentage] = useState(65);
  const [riskStudents, setRiskStudents] = useState(25);
  const [facultyScore, setFacultyScore] = useState(8.4);

  // ✅ Chart States (scaled for 100 students)
  const [performanceData, setPerformanceData] = useState([
    { grade: "A", students: 25 },
    { grade: "B", students: 30 },
    { grade: "C", students: 20 },
  ...
  // ✅ Run Analysis Function (scaled logic)
  const runAnalysis = () => {
    setIsAnalyzing(true);

    setTimeout(() => {
      setTotalStudents(100);
      setPassPercentage(78);
      setRiskStudents(18);
      setFacultyScore(9.0);
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="flex">
        ...
        <div className="flex-1 p-10 bg-gray-100">
          ...
          <div className="grid md:grid-cols-4 sm:grid-cols-2 grid-cols-1 gap-6 mb-10">
            <div className="bg-white p-6 rounded-2xl shadow-md border">
              <h3 className="text-sm text-gray-500">Total Students (CE Focus)</h3>
              <p className="text-2xl font-bold text-blue-700 mt-2">{totalStudents}</p>
            </div>

            <div className="bg-white p-6 rounded-2xl shadow-md border">
              <h3 className="text-sm text-gray-500">Pass Percentage (CE)</h3>
              <p className="text-2xl font-bold text-green-600 mt-2">{passPercentage}%</p>
            </div>

            <div className="bg-white p-6 rounded-2xl shadow-md border">
              <h3 className="text-sm text-gray-500">At-Risk Students (CE)</h3>
              <p className="text-2xl font-bold text-red-600 mt-2">{riskStudents}</p>
            </div>

            <div className="bg-white p-6 rounded-2xl shadow-md border">
              <h3 className="text-sm text-gray-500">Faculty Performance Score (CE)</h3>
              <p className="text-2xl font-bold text-purple-600 mt-2">
                {facultyScore} / 10
              </p>
            </div>
          ...
          <div className="bg-white p-6 rounded-2xl shadow-md border mb-10">
            ...
            <p className="text-gray-700 leading-relaxed">
              The AI model analyzed a dataset of{" "}
              <span className="font-semibold text-blue-700">100 students</span>{" "}
              in Computer Engineering.
              Approximately{" "}
              <span className="font-semibold text-red-600">
                {riskPercentage}% of students
              </span>{" "}
              are at academic risk.
            </p>

            <p className="text-gray-700 mt-3 leading-relaxed">
              Faculty performance shows improvement in teaching outcomes.
            </p>

            <div className="mt-5 flex items-center gap-3">
              <span className={`${riskColor} px-3 py-1 rounded-full text-sm font-medium`}>
                Risk Level: {riskLevel}
              </span>
            </div>
          </div>
